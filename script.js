...
// ==== Логика для светового пятна курсора ======
  ...
  if (!highlighter || tabs.length === 0) return;  //→ если элемент-подсветка не найден в DOM (querySelector вернул null). tabs.length === 0 → если кнопок-вкладок нет
  ...
  // ===== Показ скриншота Website на мобильных (инлайн-раскрытие под ссылкой) =====
  const mqTouchOrNarrow = window.matchMedia('(hover: none), (pointer: coarse), (max-width: 768px)').matches;
  const mqSmallScreen = window.matchMedia('(max-width: 576px)').matches;
  const prefersInlinePreview = mqTouchOrNarrow && !mqSmallScreen;
  if (prefersInlinePreview) {
     const siteLinks = document.querySelectorAll('.image-tooltip-trigger');
     siteLinks.forEach(link => {
        const hoverImg = link.querySelector('.tooltip-image-preview');
        if (!hoverImg) return;

        // Создаём контейнер предпросмотра сразу после ссылки
        const preview = document.createElement('div');
        preview.className = 'mobile-website-preview';
        const imgEl = document.createElement('img');
        imgEl.src = hoverImg.src;
        imgEl.alt = hoverImg.alt || 'Website Vorschau';
        preview.appendChild(imgEl);
        preview.style.display = 'none';
        link.parentNode.insertBefore(preview, link.nextSibling);

        // Тоггл по тапу на ссылку
        link.addEventListener('click', (e) => {
           e.preventDefault();
           const isHidden = preview.style.display === 'none';
           // Скрываем другие превью
           document.querySelectorAll('.mobile-website-preview').forEach(p => { if (p !== preview) p.style.display = 'none'; });
  ...
  const scrollUp = document.querySelector('.arrow-container');
